...
'Change COLUMN HEADER SECTION'
    ...
    'AutoFit G-L Columns on Worksheet
...
Sub YearlyChange()
    ...
    ' Find start and end rows with unique Stock Symbol
    ...
    For i = 2 To LastRow1
      ...
      StockStartRow = Range("A:A").Find(what:=Cells(i, 9), after:=Cells(1, 1)).Row
      StockEndRow = Range("A:A").Find(what:=Cells(i, 9), after:=Cells(1, 1), SearchDirection:=xlPrevious).Row

      ' Print the Change in the Summary Table
      Range("J" & Summary_Table_Row).Value = Range("C" & StockStartRow).Value - Range("F" & StockEndRow).Value
